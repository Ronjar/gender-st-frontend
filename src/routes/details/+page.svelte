    ...
    const handleSubmit = async (event: Event) => {
      ...
      try {
        const response = await fetch(`${BASE_URL}/adduser`, {
          ...
          body: JSON.stringify({ gender, age, studyProgram }),
        });
  ...
  <div class="p-6 max-w-4xl mx-auto bg-base-200 rounded-xl shadow-md space-y-4">
    ...
    <form on:submit={handleSubmit} class="space-y-4">
      ...
      <div class="form-control">
        ...
        <input type="number" bind:value={age} class="input input-bordered" min="1" max="120" required />
      </div>
