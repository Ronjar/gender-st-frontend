<script lang="ts">
  ...
  const questions: string[] = [
    ...
    'I believe I can succeed at most any <HoverTranslate word="st_ngse_endeavor" /> to which I set my mind.',
      ...
      {#each questions as question, index}
        <div class="card bg-base-300 p-4 rounded-lg shadow-md">
          <div class="form-control">
            <label class="label">
              <span class="label-text text-lg">{index + 1}. {#if question.includes("<HoverTranslate")}
                <HoverTranslate word="st_ngse_endeavor" />
              {:else}
                {question}
              {/if}</span>
            </label>
